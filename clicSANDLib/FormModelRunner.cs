...
namespace clicSANDLib
{
    ...
    static void ConvertResults(string input, string output_dir)
    {
        // Use ProcessStartInfo class
        ProcessStartInfo startInfo = new ProcessStartInfo();
        startInfo.CreateNoWindow = false;
        startInfo.UseShellExecute = false;
        startInfo.FileName = "/Users/alexanderkell/GitHub/SGI/1-cliSANDMac/converter/dist/python_converter";
        startInfo.WindowStyle = ProcessWindowStyle.Hidden;
        startInfo.Arguments = "-f j -o \"" + input + "\" -z 1.0 -s y " + output_dir;

        try
        {
            using (Process exeProcess = Process.Start(startInfo))
            {
                extBoxOutput.Text += "Running python converter";
